  ...
  while (unTraversed.length) {
    ...
    for (let i = 0; i < neighbors.length; i += 1) {
      ...
      if (!isInQueue(neighbors[i], unTraversed)) {
        ...
        const neighbour = neighbors[i]; // Get the neighbor tile
        neighbour.distance = tile.distance + 1; // Update the neighbor's distance
        neighbour.parent = tile; // Set the neighbor's parent to the current tile
        unTraversed.push(neighbour); // Add the neighbor to the queue
      }
    }
  }
